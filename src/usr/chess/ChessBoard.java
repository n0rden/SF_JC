...
/**
 * ¬ этом классе есть:
 * ѕоле board, которое представл€ет собой двумерный массив объектов шахматных фигур.
 * ѕеременна€ nowPlayer, котора€ показывает, чей сейчас ход.
 *  онструктор и метод nowPlayerColor (на самом деле хорошо бы сделать переменную nowPlayer
 * приватной, но дл€ удобства пусть будет, как есть).
 * ћетод moveToPosition, который передвигает фигуры, можете поподробнее посмотреть, что же там происходит.
 * ћетод printBoard, который просто красиво печатает шахматное поле в консоль.
 */
